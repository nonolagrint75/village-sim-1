 ...
 * Staging only: reference implementation sketch, not wired live.
 *
  ...
  for (let dy = 0; dy < DIG_SCAR.footprintH; dy++) {
    for (let dx = 0; dx < DIG_SCAR.footprintW; dx++) {
      ...
      for (const layer of DIG_SCAR.layers) {
        if (layer.centerOnly && !center) continue
...
/** Suggested call site (documentation only — do not wire in this pack). */
export const INTEGRATOR_HOOK = `
// After drawNatureCloseup(...), before NPC pass:
import { drawMineVisuals } from "_staging/visual/mines_ore/art/drawApi" // path TBD when promoting
import { drawGroundShadow, drawSouthFace, drawNorthLit } from "@/lib/render/fauxHeight"
import { getNatureAtlas } from "@/lib/render/nature/loader"
import { hash2 } from "@/lib/render/nature/textureLab"

const sites = villages
  .filter(v => v.hasMine && v.mineX >= 0)
  .map(v => ({ gx: v.mineX, gy: v.mineY, ore: detectNearbyOre(...), active: minersNearby(...), pileCount: 2 }))

drawMineVisuals(ctx, getNatureAtlas()!, { drawGroundShadow, drawSouthFace, drawNorthLit },
  sites, camX, camY, zoom, tilePx, hash2)

// Also: stop filling GOLD/IRON cells with full-tile ore_* in drawNatureCloseup;
// keep mountain_plateau / grass ground and emit ore_pile props instead.
` as const
