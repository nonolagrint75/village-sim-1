...
function chooseTask(state: SimState, v: Villager, rng: () => number) {
  ...
  // Hard gate: colony survival beats troubadours — entertain/counsel wiped food loops.
  {
    ...
    // Hard gate: no troubadour/counsel loops while starving, under famine, after dark, or with a field waiting.
    const canServe =
      !famine &&
      !night &&
      v.hunger >= 2.35 &&
...
function executeTask(state: SimState, v: Villager, rng: () => number): boolean {
  ...
  if (task.kind === 'craftStoneSpear' && arrived) {
...
export function tickVillager(state: SimState, v: Villager, rng: () => number) {
  ...
  // Hearth recovers a little even without a rest task.
  {
    ...
    if (!atHomeShelter(v) && !v.embarked && !byFire && (cold > 0.05 || heat > 0.05 || rain > 0.4)) {
      ...
      if (!earlyFounding && cold > 0.28 && exposure > 0.34 && (state.tick + v.id * 7) % 4 === 0) {
        if (deficit > 0.45) {
