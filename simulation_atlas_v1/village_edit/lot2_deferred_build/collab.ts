...
export function isActiveHomeSite(owner: Villager): boolean {
  ...
  return owner.buildQueue.some((b) => !b.done)
}
...
export function listOpenBuildSites(state: SimState, helper: Villager, maxDist = COLLAB_SITE_RADIUS): OpenBuildSite[] {
  ...
  for (const owner of state.villagers) {
    ...
    if (owner.hunger < 1.1 || owner.thirst < 1.0) continue
...
export function helperSurvivalOk(helper: Villager): boolean {
  if (helper.hunger < 1.35 || helper.thirst < 1.2) return false
  if (helper.starveTimer > 4 || helper.thirstTimer > 4) return false
...
export function scoreHelpBuild(
  ...
  if (!helper.hasHome && helper.house && helper.buildQueue.some((b) => !b.done)) {
    urge *= 0.35
  }
  // Reach falloff.
...
export function placeBlockForOwner(
  ...
  let block =
    owner.buildQueue.find((b) => !b.done && b.x === targetX && b.y === targetY) ??
    nextBuildBlock(owner.buildQueue, state.grid) ??
    null
  if (!block) return { ok: false, block: null, structLeft: structuralBlocksRemaining(owner.buildQueue) }

  // Age gate: children never place heavy structure.
  if (helper.age < 14 && HEAVY_BLOCK.has(block.kind)) {
    return { ok: false, block, structLeft: structuralBlocksRemaining(owner.buildQueue) }
  }
  ...
  markBlockDone(owner.buildQueue, block.x, block.y, block.kind)

  return {
    ok: true,
    block,
    structLeft: structuralBlocksRemaining(owner.buildQueue),
  }
}
...
export function settleBuildHelp(
  ...
  recordRelHistory(owner, helper.id, hist, state.tick, 'chantier')
  recordRelHistory(helper, owner.id, 'helped', state.tick, 'chantier')
...
export function settleHireAdvance(state: SimState, owner: Villager, builder: Villager): boolean {
  ...
  recordRelHistory(builder, owner.id, paid ? 'gift' : 'talk', state.tick, 'embauche')
  recordRelHistory(owner, builder.id, 'helped', state.tick, 'embauche')
  remember(builder, {
    kind: 'helped',
    subjectId: owner.id,
    x: owner.x,
