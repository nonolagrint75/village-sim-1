...
p = Path(r"C:\Users\kamel\village-sim-1\simulation_atlas_v1\village_edit\src\lib\render\nature\shorePaint.ts")
log = Path(r"C:\Users\kamel\village-sim-1\simulation_atlas_v1\village_edit\_patch_log.txt")
c = p.read_text(encoding="utf-8")
out = [f"len={len(c)}"]
start = c.find("function domainWarp")
assert start >= 0, "no domainWarp"
...
while i < len(c):
    if c[i] == "{":
        ...
        if brace == 0:
            ...
            break
    i += 1
out.append(f"comment_start={comment_start} end_fn={end_fn}")
new_warp = """/** Multi-octave vector domain-warp - hard 0.5 isocontour must leave tile mid-edges. */
...
"""
c = c[:comment_start] + new_warp + c[end_fn:]
reps = [
 ...
 ("HARD f>=0.5 water gate + strong vector domain-warp so isocontour leaves tile mid-edges.",
  "HARD f>=0.5 water gate + WARP_V2_AGGRESSIVE multi-octave domain-warp so isocontour leaves tile mid-edges."),
 ("fbm2(wx * 0.5 + 9.1, wy * 0.5 - 3.4, 4, 2.1, 0.55) * 0.95",
  "fbm2(wx * 0.42 + 9.1, wy * 0.42 - 3.4, 5, 2.1, 0.55) * 1.45"),
 ("simplex2(wx * 1.45 + 2.2, wy * 1.45 - 7.7) * 0.65",
  "simplex2(wx * 1.15 + 2.2, wy * 1.15 - 7.7) * 0.95"),
 ("simplex2(wx * 2.9 - 4.4, wy * 2.9 + 1.3) * 0.38",
  "simplex2(wx * 2.4 - 4.4, wy * 2.4 + 1.3) * 0.55"),
 ...
 ("simplex2(wx * 1.95 + 3.3, wy * 1.95 - 1.8) * 0.5 * mid",
  "simplex2(wx * 1.55 + 3.3, wy * 1.55 - 1.8) * 0.75 * mid"),
 ("shoreWarp(sx + 2.4, sy + 1.1) * 0.95 * edge",
  "shoreWarp(sx + 2.4, sy + 1.1) * 1.35 * edge"),
]
for a,b in reps:
    n = c.count(a)
    out.append(f"rep {n}: {a[:40]}")
    c = c.replace(a, b)
p.write_text(c, encoding="utf-8")
out.append(f"WARP_V2={('WARP_V2_AGGRESSIVE' in c)}")
out.append(f"amp={('* 3.4' in c)}")
out.append(f"f05={('if (f < 0.5)' in c)}")
out.append(f"pad3={c.count('minX - 3')}")
out.append(f"touch4={c.count('gy, 4)')}")
