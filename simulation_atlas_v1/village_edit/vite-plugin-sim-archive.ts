        ...
        req.on('end', () => {
          try {
            ...
            if (archive?.id) {
              ...
              // Human-readable full dump next to the JSON.
              const full = archive as {
                id?: string
                seed?: number
                startedAt?: string
                endedAt?: string
              ...
              const txt: string[] = []
              txt.push(`=== Village Sim — logs complets ===`)
              txt.push(`run: ${full.id ?? ''}`)
              txt.push(`seed: ${full.seed ?? ''}`)
              txt.push(`startedAt: ${full.startedAt ?? ''}`)
              txt.push(`endedAt: ${full.endedAt ?? ''} (${full.endReason ?? ''})`)
              txt.push('')
              txt.push('========== RÉSUMÉ ==========')
              txt.push(JSON.stringify(full.summary ?? {}, null, 2))
              txt.push('')
              txt.push(`========== CHRONIQUE (${(full.chronicleTail ?? []).length} lignes) ==========`)
              for (const line of full.chronicleTail ?? []) txt.push(line)
              txt.push('')
              txt.push(`========== SÉRIE (${(full.series ?? []).length} jours) ==========`)
              if ((full.series ?? []).length) {
                const keys = Object.keys(full.series![0]!)
                txt.push(keys.join('\t'))
                for (const row of full.series ?? []) {
                  txt.push(keys.map((k) => String(row[k] ?? '')).join('\t'))
                }
              }
              fs.writeFileSync(path.join(logDir, `${archive.id}.txt`), txt.join('\n'), 'utf8')
              fs.writeFileSync(path.join(logDir, 'latest.txt'), txt.join('\n'), 'utf8')
            }
            ...
            for (const name of fs.readdirSync(logDir)) {
              if (!keep.has(name) && name.startsWith('run_') && (name.endsWith('.json') || name.endsWith('.txt'))) {
                try {
