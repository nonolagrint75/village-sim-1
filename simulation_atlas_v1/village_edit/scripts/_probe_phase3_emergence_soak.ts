/**
 * PHASE 3 - Natural emergence soak at sec20 individual floors.
 * NO induced famine / profession / CREATE_*. stepSimulation only.
 *
 *   npx tsx scripts/_probe_phase3_emergence_soak.ts [days=60] [seeds=1,3,7]
 *
 * Writes: _phase3_soak_s{seed}.json, _phase3_soak_summary.json, _phase3_soak_run.txt
 */
...
const RUN_LOG = path.join(ROOT, '_phase3_soak_run.txt')
      ...
      if (day % 10 === 0 || day === days) {
        console.log(
          `  seed=${seed} d${day} alive=${alive} deaths=${state.deaths} leaves=${state.migrationCounters?.leaves ?? 0} ` +
            `taskDeltaPROXY=${taskKindChanges} profDelta=${professionChanges}`,
        )
      }
    }
  }
...
console.log('=== PHASE 3 NATURAL EMERGENCE SOAK ===')
...
for (const seed of seeds) {
  ...
  try {
    ...
    rows.push(dump)
    const outPath = path.join(ROOT, `_phase3_soak_s${seed}.json`)
    ...
    rows.push(crashDump)
    fs.writeFileSync(path.join(ROOT, `_phase3_soak_s${seed}.json`), JSON.stringify(crashDump, null, 2), 'utf8')
...
const summaryPath = path.join(ROOT, '_phase3_soak_summary.json')
