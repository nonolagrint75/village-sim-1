...
import { noteSimTps } from '../src/lib/sim/perfBudget'
...
function cloneVillager(src: Villager, id: number, dx: number, dy: number): Villager {
  ...
  return {
    ...
    activeProjectId: null, knowledge: [],
...
for (const pop of [50, 120, 250, 500]) {
  const state = createSimulation(7, { preset:'vast', worldSize:1000, initialVillagers: Math.min(120, pop), maxPopulation:500, wolfCount:1 })
  if (pop > state.villagers.length) fill(state, pop)
  setAliveAgentCount(pop)
  resetPathBudget(pop)
  agentHash.rebuild(state)
  // one full tickVillager on first agent only, timed
  const v = state.villagers.find(x => x.alive)!
  const t0 = performance.now()
  for (let i = 0; i < 20; i++) tickVillager(state, v, () => 0.4, false)
  const ms = (performance.now() - t0) / 20
  console.log(JSON.stringify({ pop, msPerFullTickOneAgent: +ms.toFixed(3) }))
}