/**
 * PHASE 4 CP5 - Natural causality/emergence re-soak at sec20 individual floors.
 * NO induced famine / profession / CREATE_*. stepSimulation only.
 *
 *   npx tsx scripts/_probe_phase4_causality_soak.ts [days=60] [seeds=1,3,7]
 *
 * Writes: _phase4_soak_s{seed}.json, _phase4_soak_summary.json, _phase4_soak_run.txt
 */
...
const RUN_LOG = path.join(ROOT, '_phase4_soak_run.txt')
...
function emptyAttributionSnap(): ReturnType<typeof snapshotAttribution> {
  return {
    ...
    personalityPairSamples: 0, personalityPairReady: 0, status: 'NOT_TESTED',
  }
}
...
function runSeed(seed: number): SeedDump {
  ...
  const helpCategoryCount = Object.values(help).filter((n) => n > 0).length
...
console.log('=== PHASE 4 CP5 NATURAL CAUSALITY SOAK ===')
...
for (const seed of seeds) {
  ...
  try {
    ...
    rows.push(dump)
    const outPath = path.join(ROOT, `_phase4_soak_s${seed}.json`)
    ...
    rows.push(crashDump)
    fs.writeFileSync(path.join(ROOT, `_phase4_soak_s${seed}.json`), JSON.stringify(crashDump, null, 2), 'utf8')
...
const totals = {
  ...
  attribution: {
    ...
    personalityAttributedDecisions: okRows.reduce((a, r) => a + r.attribution.personalityAttributedDecisions, 0),
  ...
  causalitySec22BySeed: okRows.map((r) => ({ seed: r.seed, sec22Status: r.causality.sec22Status, chains: r.causality.chains })),
}
...
const summaryPath = path.join(ROOT, '_phase4_soak_summary.json')
