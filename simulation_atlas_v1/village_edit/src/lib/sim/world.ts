...
function placeGeologicalResources(grid: WorldGrid, elevation: Float32Array, moisture: Float32Array, temperature: Float32Array, rng: () => number, mountainLevel: number) {
  ...
  for (let y=2;y<grid.height-2;y++) for (let x=2;x<grid.width-2;x++) {
    ...
    // Slightly denser soft metals so mid-run prospecting + mine mouths emerge more often.
    if (oreNoise > 0.52 && softVein > 0.46) grid.copperDeposit[i] = 6 + Math.floor(oreNoise * 24)
    if (oreNoise > 0.68 && softVein > 0.55 && rng() < 0.48) grid.tinDeposit[i] = 3 + Math.floor(oreNoise * 12)
    if (oreNoise > 0.6 && softVein < 0.45) grid.leadDeposit[i] = 4 + Math.floor(oreNoise * 14)
    if (oreNoise > 0.8 && veinNoise > 0.65 && rng() < 0.38) grid.silverDeposit[i] = 2 + Math.floor(oreNoise * 8)
    if (moisture[i] > 0.32 && oreNoise > 0.5 && softVein > 0.42 && rng() < 0.6) {
      grid.coalDeposit[i] = 5 + Math.floor(oreNoise * 20)
    }
    if (oreNoise > 0.62 && veinNoise > 0.45) grid.ironDeposit[i] = Math.max(grid.ironDeposit[i], 6 + Math.floor(oreNoise * 22))
  }
  ...
  for (let y = 3; y < grid.height - 3; y++) {
    for (let x = 3; x < grid.width - 3; x++) {
      ...
      if (outcrop > 0.78 && rng() < 0.045) setTerrain(grid, x, y, IRON, 6 + Math.floor(outcrop * 14))
      else if (outcrop > 0.86 && rng() < 0.016) setTerrain(grid, x, y, GOLD, 3 + Math.floor(outcrop * 8))
    }
  }
}
