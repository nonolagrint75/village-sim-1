...
import { distance } from '@/lib/sim/world'

/** Match `HOME_REST_RADIUS` in behaviors — « chez lui » only when this close to the plot. */
const HOME_REST_RADIUS = 2
/** Rest target counts as home-bound within this of homeX/homeY (bed / hearth). */
const HOME_TARGET_RADIUS = 4.5
...
export const TASK_LABELS: Record<TaskKind, string> = {
  ...
  helpBuild: 'Aide à bâtir',
  haulForBuild: 'Apporte des matériaux de chantier',
  assistCraftTools: 'Aide à forger des outils',
  hireBuilder: 'Embauche un bâtisseur',
}

/** Contextual French label for an active rest task (portrait / SimPanel). */
export function restTaskLabel(v: {
  hasHome: boolean
  x: number
  y: number
  homeX: number
  homeY: number
  task: { kind: string; targetX: number; targetY: number } | null
}): string {
  if (v.task?.kind !== 'rest') return TASK_LABELS.rest
  const atHome = v.hasHome && distance(v.x, v.y, v.homeX, v.homeY) <= HOME_REST_RADIUS
  const homeBound =
    v.hasHome && distance(v.task.targetX, v.task.targetY, v.homeX, v.homeY) <= HOME_TARGET_RADIUS
  // Match gameplay sheltered rest (at plot + home-bound target). Never claim « chez lui »
  // for outdoor-collapse targets even if the body happens to stand near the door.
  if (homeBound && !atHome) return 'Rentre se reposer'
  if (atHome && homeBound) return 'Se repose chez lui'
  return 'Se repose dehors'
}
