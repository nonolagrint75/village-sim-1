...
import * as Tech from './technology'
...
export function enqueueBuildProject(
  ...
  const tech = {
    highKeep: Tech.knowsHighStoneKeep(inventor, village),
    arrowSlit: Tech.knowsArrowSlit(inventor, village),
  }
...
export function projectTaskUrge(project: BuildProject, p: Personality): number {
  ...
  if (project.intent.purposes.includes('fortify')) urge += 48 + (1 - p.courage) * 28
  ...
  if (project.phase === 'build') urge += 22
...
export function nextConstructionStep(
  ...
  if (have < TILE_COST) {
    ...
    const kindRes = res === 'stone' ? 'stone' : 'tree'
    const near =
      nearestResource(grid, project.cx, project.cy, kindRes, 52) ??
      nearestResource(grid, project.cx, project.cy, kindRes, 96)
    if (near) {
  ...
  if (project.intent.purposes.includes('fortify')) {
    ...
    if (allWalls > 0 && stamped / allWalls >= 0.62) {
...
/** Empty village shell used by pioneer founding and leave→foundCamp (INC-04). */
export function emptyFoundingVillage(state: SimState, x: number, y: number, rng: () => number): Village {
...
/**
 * INC-04: housed leaver secedes hearth into a new camp (leave→foundCamp)
 * instead of villageId=null → instant rejoin.
 */
export function foundMigrateCamp(
  state: SimState,
  v: Villager,
  rng: () => number,
): Village {
  const x = v.homeX >= 0 ? v.homeX : Math.round(v.x)
  const y = v.homeY >= 0 ? v.homeY : Math.round(v.y)
  const camp = emptyFoundingVillage(state, x, y, rng)
  if (!camp.memberIds.includes(v.id)) camp.memberIds.push(v.id)
  v.villageId = camp.id
  return camp
}
...
export function seedPioneerCamps(
  ...
  for (let g = 0; g < foundingGroups; g++) {
    ...
    for (const v of pioneers) {
      ...
      // One pioneer farm per founding village — not a mini-field on every cabin.
      // Seeds growth for the mill chain; profession locks to farmer so crafts don't inherit the plot.
      if (v.fieldX === -1 && placed === 0) {
        const field =
          findBuildSite(grid, plot.x - 10, plot.y, FIELD_RADIUS, 30, 8) ??
          findBuildSite(grid, plot.x + 10, plot.y + 2, FIELD_RADIUS, 26, 8)
        if (field) {
          v.fieldX = field.x
          v.fieldY = field.y
          {
          ...
          claimArea(grid, field.x, field.y, FIELD_RADIUS, CLAIM_FIELD)
          const cells = fieldCells(grid, field.x, field.y, FIELD_RADIUS)
    ...
    // Affiliation before own cabin — keeps founding cluster from diaspora-wandering.
    for (const v of members) {
      if (v.villageId === village.id) continue
      v.villageId = village.id
...
// ---------------------------------------------------------------------------
// Lot 3B — progressive home path (plan → queue → BuildBlock).
// Does NOT replace BuildProject / applyConstructionStep above.
// NPC behaviors stay dormant until Lot 3C.
// ---------------------------------------------------------------------------
export { planHomeSpatial, fallbackLeanPlan } from './build/planner'
export {
  emitBuildQueue,
  nextBuildBlock,
  markBlockDone,
...
} from './build/homeBuildPipeline'
export { ensureBlockWorld } from './build/stateBridge'
export {
  buildQueueRemaining,
  buildQueueComplete,
  emptyBuildQueue,
