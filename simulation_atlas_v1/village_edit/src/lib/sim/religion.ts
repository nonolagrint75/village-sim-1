...
import { forEachLifeEvent } from './cognition/memory'
...
import { noteCircleFormed, noteConflict } from './societyMetrics'
...
function tickPietyDrift(state: SimState, v: Villager): void {
  ...
  let bump = 0
  forEachLifeEvent(peekMind(v.id)?.episodic, v.memories, { tick: state.tick, maxAge: 500 }, (kind) => {
    if (kind === 'grief' || kind === 'harmed' || kind === 'saved') bump += 0.008
  })
...
/**
 * Sole creed crystallize writer (WP4).
 * Reads pol.beliefs + mind.sacredConf; politics/stubs only accumulate weight / sacredConf.
...
export function maybeCrystallizeFaithCreed(state: SimState, v: Villager): boolean {
  ...
  const faithCircle = circlesOf(state, v).some((c) => c.kind === 'faith')
  // Faith path (piety + sacred/weight/circle) OR grievance/weight path (ex-politics crystallize).
  const ready =
    (pol.beliefs.piety >= CREED_FROM_PIETY &&
      (pol.creedWeight >= 0.24 || sacred >= 0.28 || faithCircle)) ||
    pol.grievance >= 0.4 ||
    (pol.creedWeight >= 0.28 && (pol.beliefs.piety >= 0.5 || sacred >= 0.28))
  if (!ready) return false

  let next: CreedId | null = null
  // Competitive axes — sacred sites must not always crystallise as « piété ».
  if (pol.grievance > 0.5 && pol.beliefs.piety < 0.52) {
    next = pickDominantCreed(pol, 0.48) ?? (pol.grievance > 0.55 ? 'ordre' : null)
  } else if (pol.beliefs.piety >= 0.52 || sacred >= 0.38) {
    next = pickDominantCreed(pol, 0.42)
  } else if (pol.beliefs.tradition > 0.55) next = 'tradition'
  else if (pol.beliefs.fairness > 0.55) next = 'partage'
  else if (pol.beliefs.greed > 0.55) next = 'commerce_libre'
  else if (pol.grievance > 0.4) next = pickDominantCreed(pol, 0.4) ?? 'ordre'
  else if (pol.beliefs.loyalty > 0.58 && pol.grievance > 0.28) next = 'protection'
  else if (pol.beliefs.piety >= CREED_FROM_PIETY) next = pickDominantCreed(pol, 0.4) ?? 'piete'
...
/**
 * Soft schism: when a faith circle holds two competing creeds, the minority
 * peels off into its own cercle (no CREATE_RELIGION — divergence from belief conflict).
 */
function maybeFaithSchism(state: SimState, c: Circle): void {
  if (c.kind !== 'faith') return
  if ((state.tick + c.id * 13) % 120 !== 0) return
  const members = livingMembers(state, c.memberIds)
  if (members.length < 4) return
  const byCreed = new Map<CreedId, Villager[]>()
  for (const m of members) {
    const cr = politicsOf(m).creed
    if (!cr) continue
    let list = byCreed.get(cr)
    if (!list) {
      list = []
      byCreed.set(cr, list)
    }
  ...
  if (byCreed.size < 2) return
  const ranked = [...byCreed.entries()].sort((a, b) => b[1].length - a[1].length)
  const [majCreed, maj] = ranked[0]!
  const [minCreed, min] = ranked[1]!
  if (min.length < 2 || maj.length < 2) return
  const tense =
    min.some((m) => politicsOf(m).grievance > 0.32) ||
    c.cohesion < 0.42 ||
    Math.abs(politicsOf(maj[0]!).beliefs.tradition - politicsOf(min[0]!).beliefs.tradition) > 0.25
  if (!tense) return

  for (const m of min) {
    c.memberIds = c.memberIds.filter((id) => id !== m.id)
  }
  c.cohesion = clamp01(c.cohesion - 0.18)
  if (!c.creed || c.creed === minCreed) c.creed = majCreed

  const schism: Circle = {
    id: state.nextCircleId++,
    name: `cercle — ${CREED_FR[minCreed]}`,
    kind: 'faith',
    memberIds: min.map((m) => m.id),
    values: {
      fairness: 0.5,
      loyalty: 0.45,
      greed: 0.4,
      piety: 0.55,
      tradition: politicsOf(min[0]!).beliefs.tradition,
    },
    reputation: 0.35,
    leaderId: min[0]!.id,
    legitimacy: 0.3,
    ...
    problemCount: 1,
    isInstitution: false,
    originStory: `schisme depuis ${c.name}`,
    memory: [`rupture sur « ${CREED_FR[minCreed]} »`],
    pooledFood: 0,
    norms: [],
    creed: minCreed,
    ...
    cohesion: 0.35,
    enforcement: 0.12,
    authorityBasis: 'charisma',
    techIds: [],
    isGuild: false,
    qualityBar: 0.35,
  }
  state.circles.push(schism)
  noteCircleFormed(state)
  noteConflict(state, 'schism')
  logCause(
    state,
    `désaccord de foi dans ${c.name}`,
    `schisme : « ${CREED_FR[minCreed]} » se sépare de « ${CREED_FR[majCreed]} »`,
  )
}
