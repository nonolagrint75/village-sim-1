...
/** Real mining needs stone or iron — bare hands chip nothing useful. */
export function canMineRock(tier: ToolTier): boolean {
  return tier === 'stone' || tier === 'iron'
}

/** Soft claim dig: wood tools may chip a pre-softened claimed face only. */
export function canChipClaimedFace(tier: ToolTier): boolean {
  return tier === 'wood' || tier === 'stone' || tier === 'iron'
}
...
export function finalizeTunnelCell(
  ...
  // Claimed mouth nearby: any opened tunnel crystallizes the village mine entry.
  if (opensOut && village && village.mineX >= 0 && !village.hasMine) {
    const dx = Math.abs(village.mineX - x)
    const dy = Math.abs(village.mineY - y)
    if (dx + dy <= 3) {
      village.hasMine = true
      village.mineX = x
      village.mineY = y
...
/** Remember a tunnel mouth so crystallize can reattach after village churn. */
export function rememberMineMouth(state: SimState, x: number, y: number): void {
  if (!state.knownMineMouths) state.knownMineMouths = []
  if (state.knownMineMouths.some((m) => m.x === x && m.y === y)) return
  state.knownMineMouths.push({ x, y })
  if (state.knownMineMouths.length > 48) state.knownMineMouths = state.knownMineMouths.slice(-48)
}

/**
 * Safety crystallize: if a claimed mouth (or nearby cell) is already TUNNEL,
 * register the village mine even when dig ran without a village pointer.
 * Spec A §17 / soak: mines must emerge causally once corridors exist.
 */
export function crystallizeVillageMines(state: SimState): number {
  const grid = state.grid
  let n = 0
  // Reattach known mouths to nearest village without a mine.
  for (const mouth of state.knownMineMouths ?? []) {
    if (getTerrain(grid, mouth.x, mouth.y) !== TUNNEL) continue
    let best: Village | null = null
    let bestD = 96
    for (const village of state.villages) {
      if (village.hasMine) continue
      const d = Math.hypot(village.centerX - mouth.x, village.centerY - mouth.y)
      if (d < bestD) {
        bestD = d
        best = village
      }
  ...
  for (const village of state.villages) {
    if (village.hasMine) continue
    let cx = village.mineX
    let cy = village.mineY
    if (cx < 0 || cy < 0) {
      // Fallback: any TUNNEL entrance within settlement radius of centre.
      const r = 48
      let found: { x: number; y: number } | null = null
      for (let y = village.centerY - r; y <= village.centerY + r && !found; y++) {
        for (let x = village.centerX - r; x <= village.centerX + r; x++) {
          if (!inBounds(grid, x, y)) continue
          if (getTerrain(grid, x, y) !== TUNNEL) continue
          if (grid.amount[y * grid.width + x] < TUNNEL_ENTRANCE_AMOUNT) continue
          found = { x, y }
          break
        }
      }
    ...
    // Claimed mouth or neighbour already opened?
    ...
    if (!mouth) continue
    village.hasMine = true
    village.mineX = mouth.x
    village.mineY = mouth.y
    rememberMineMouth(state, mouth.x, mouth.y)
    n++
  }
  return n
}
