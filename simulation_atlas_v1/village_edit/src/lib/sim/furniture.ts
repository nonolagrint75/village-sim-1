...
export function nextFurnitureJob(queue: FurnitureJob[], inv?: Slot[]): FurnitureJob | null {
  const pending = queue.filter((j) => !j.done)
  if (pending.length === 0) return null
  if (!inv) return pending[0] ?? null
  // Prefer a job we can craft now so exotic recipes (clay/tallow) don't block tables forever.
  const affordable = pending.find((j) => canAffordFurniture(inv, j.kind))
  if (affordable) return affordable
  // Only resources with direct map gather tasks — linen/cloth/wool need weaving first.
  const GATHERABLE = new Set<ResourceType>(['wood', 'stone', 'iron'])
  const CRAFT_ONLY = new Set<ResourceType>(['clay', 'tallow', 'flax', 'linen', 'cloth', 'wool', 'rope'])
  const onlyNeedsGatherable = (kind: FurnitureKind): boolean => {
    const recipe = FURNITURE_DEFS[kind].recipe
    if (kind === 'bed') {
      if (countOf(inv, 'wood') < 3) return true
      return countOf(inv, 'linen') >= 1 || countOf(inv, 'cloth') >= 1 || countOf(inv, 'wool') >= 2
    }
    for (const key of Object.keys(recipe) as ResourceType[]) {
      const need = recipe[key] ?? 0
      if (need <= 0) continue
      if (countOf(inv, key) >= need) continue
      if (!GATHERABLE.has(key)) return false
    }
    return true
  }
  // Wood-only pieces (table, bench) before hearth's stone→clay→tallow chain.
  const gatherableJob = pending.find((j) => onlyNeedsGatherable(j.kind))
  if (gatherableJob) return gatherableJob
  for (const j of pending) {
    const missing = missingFurnitureResource(inv, j.kind)
    if (!missing || GATHERABLE.has(missing)) return j
  }
  // Skip craft-chain blocked jobs — try a later wood-only piece (table, bench, stool).
  for (const j of pending) {
    const missing = missingFurnitureResource(inv, j.kind)
    if (missing && CRAFT_ONLY.has(missing)) continue
    return j
  }
  return pending[0] ?? null
}

export function markFurnitureDone(
  queue: FurnitureJob[],
  kind: FurnitureKind,
  x: number,
  y: number,
): FurnitureJob | null {
  const exact = queue.find((j) => !j.done && j.kind === kind && j.x === x && j.y === y)
  if (exact) {
    exact.done = true
    return exact
  }
  const sameKind = queue.find((j) => !j.done && j.kind === kind)
  if (sameKind) {
    sameKind.done = true
    sameKind.x = x
    sameKind.y = y
...
export function applyFurnitureBuilt(v: Villager, kind: FurnitureKind, x: number, y: number): FurnitureJob | null {
  ...
  return markFurnitureDone(v.furnitureQueue, kind, x, y)
}
