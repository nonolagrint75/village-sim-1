...
const FOOD_PER_CAPITA = 2.0
/** Match behaviors.ts HUNGRY_THRESHOLD — local famine, not global oracle. */
const VILLAGE_HUNGRY_THRESHOLD = 2

function villageFoodStores(village: Village): number {
  return (
...
/** Per-village famine: local hunger ratio or pantry crisis (mirrors tickFamine, scoped). */
export function villageInFamine(state: SimState, village: Village): boolean {
  let alive = 0
  let hungry = 0
  let stores = villageFoodStores(village)
  for (const id of village.memberIds) {
    const m = state.villagers.find((x) => x.id === id && x.alive)
    if (!m) continue
    alive++
    if (m.hunger < VILLAGE_HUNGRY_THRESHOLD) hungry++
    stores += edibleValue(m.inventory) * 0.25
    if (m.chestInventory) stores += edibleValue(m.chestInventory) * 0.35
  }
  if (alive === 0) return false
  const stockCrisis = state.tick >= TICKS_PER_DAY * 3 && stores < alive * 0.28
  return hungry / alive > 0.6 || stockCrisis
}

/**
 * What an agent / village should *feel* as famine for decisions.
 * Global `state.famine` OR local pantry/hunger crisis — same OR used by careers.ts.
 * Prefer this over raw `state.famine` in chooseTask / leisure / profession stickiness.
 */
export function feelFamine(state: SimState, village: Village | null | undefined): boolean {
  if (state.famine) return true
  if (village && village.memberIds.length > 0) return villageInFamine(state, village)
  return false
}

/** Resolve villager's home village then feelFamine (null village → global only). */
export function villagerFeelsFamine(state: SimState, v: Villager): boolean {
  if (v.villageId == null) return !!state.famine
  const vg = state.villages.find((g) => g.id === v.villageId)
  return feelFamine(state, vg)
}
