...
import { inheritOnDeath, recordHelp, sameFamily } from './family'
import { noteDefendHelpTaken, noteHelpOutcome, noteTeachCraftEvent } from './causalityMetrics'
...
import { cleanupDeadBuilder } from './build/npcBuildBehaviors'
import { releaseAssigneeOrders } from './build/workOrders'
import { CLAIM_NONE, LOOT, type SimState, type Villager } from './types'
import { teachKnowledge } from './technology'
import { noteActivityPractice, notePatronage, noteRecognition } from './livelihood'
import { FIELD_RADIUS } from './fields'
import { distance, makeRng, claimArea, claimCells, inBounds, isBuildableGround, setTerrain } from './world'
...
/** Copy house shell, layout, and furniture queue when ownership passes to an heir. */
function transferHomeState(from: Villager, to: Villager): void {
  to.hasHome = true
  to.homeX = from.homeX
  to.homeY = from.homeY
  to.homeOwnerId = to.id
  to.house = from.house
  to.homeLayout = from.homeLayout
  to.furnitureQueue = from.furnitureQueue
  to.hasWorkbench = from.hasWorkbench
  to.workbenchX = from.workbenchX
  to.workbenchY = from.workbenchY
  to.hasChest = from.hasChest
  to.chestX = from.chestX
  to.chestY = from.chestY
  to.chestInventory = from.chestInventory
  to.hasTable = from.hasTable
  to.tableX = from.tableX
  to.tableY = from.tableY
  to.bedCount = from.bedCount
  to.homeFurniture = from.homeFurniture
...
export function doSocialise(state: SimState, a: Villager, b: Villager) {
  ...
  recordRelHistory(a, b.id, 'gossip', state.tick)
  recordRelHistory(b, a.id, 'gossip', state.tick)
  ...
  // Soft craft imitation — watching a skilled peer drips practice (not only teachCraft).
  const craftGapA = mb.skills.craft - ma.skills.craft
  const craftGapB = ma.skills.craft - mb.skills.craft
  if (craftGapA > 0.1 && rngA() < 0.4) {
    ma.skills.craft = Math.min(1, ma.skills.craft + 0.01 + mb.skills.social * 0.005)
    noteActivityPractice(a, 'teachCraft', 0.18)
  }
  if (craftGapB > 0.1 && rngB() < 0.4) {
    mb.skills.craft = Math.min(1, mb.skills.craft + 0.01 + ma.skills.social * 0.005)
    noteActivityPractice(b, 'teachCraft', 0.18)
  }
...
export function doGiveFood(state: SimState, giver: Villager, receiver: Villager): boolean {
  ...
  recordHelp(state, 'food', giver.id)
  noteHelpOutcome(state, 'food', giver.id)
  const kinShare =
    sameFamily(giver, receiver) ||
...
export function doTeachCraft(state: SimState, master: Villager, pupil: Villager): void {
  ...
  const skillBefore = pm.skills[best]
  let skillChanged = false
  if (bestV > pm.skills[best] + 0.05) {
    pm.skills[best] = Math.min(1, pm.skills[best] + 0.02 + mm.skills.social * 0.01)
    skillChanged = true
  }
  const skillAfter = pm.skills[best]
  // Receipt only when skill really dripped (not social bump on master alone).
  noteTeachCraftEvent(state, {
    pupilId: pupil.id,
    teacherId: master.id,
    skillChanged,
    skillKey: skillChanged ? String(best) : undefined,
    skillBefore,
    skillAfter,
  })
  mm.skills.social = Math.min(1, mm.skills.social + 0.006)
  teachKnowledge(state, master, pupil, makeRng(((state.tick * 7919 + master.id * 104729 + pupil.id) >>> 0) || 1))
  ...
  recordHelp(state, 'teach', master.id)
  if (skillChanged) noteHelpOutcome(state, 'teach', master.id)
...
export function doConfront(state: SimState, aggressor: Villager, target: Villager, rng: () => number) {
  ...
  state.brawls += 1
  // WP10 conflict taxonomy — cheap cause inference from rumors / grudge / creed.
  const theftFeud = state.rumors.some(
    (r) =>
      (r.kind === 'theft' || r.kind === 'exclusion') &&
      (r.aboutId === target.id || r.aboutId === aggressor.id) &&
      r.intensity > 0.25,
  )
  const rivalry = state.rumors.some(
    (r) => r.kind === 'rivalry' && (r.subjectId === aggressor.id || r.aboutId === target.id),
  )
  if (theftFeud) noteConflict(state, 'theft_feud')
  else if (rivalry) noteConflict(state, 'rivalry')
  else noteConflict(state, 'confront')
...
/** Visual §57 / Sim ground goods — leftover inventory spills as LOOT piles (sim-backed). */
export function dropGroundGoods(state: SimState, victim: Villager, fraction = 0.45): number {
  const DROP_KINDS: ResourceType[] = [
    'food',
    'wood',
    'stone',
    'iron',
  ...
  for (const kind of DROP_KINDS) {
    const n = countOf(victim.inventory, kind)
    if (n <= 0) continue
    const take = Math.max(1, Math.floor(n * fraction))
    total += removeFromInventory(victim.inventory, kind, take)
  }
  if (total <= 0) return 0
  ...
  for (const [dx, dy] of offsets) {
    ...
    if (!isBuildableGround(grid, x, y) && grid.terrain[y * grid.width + x] !== LOOT) continue
    const pile = Math.min(remaining, Math.max(1, Math.ceil(remaining / 3)))
    const i = y * grid.width + x
    const prev = grid.terrain[i] === LOOT ? grid.amount[i] ?? 0 : 0
    setTerrain(grid, x, y, LOOT, Math.min(24, prev + pile))
    remaining -= pile
  }
...
export function onDeath(state: SimState, victim: Villager, killer: Villager | null) {
  ...
  // Residual goods after inheritance → ground piles (raid/death readable without UI).
  dropGroundGoods(state, victim, killer ? 0.55 : 0.4)
  dropMind(victim.id)
  ...
  for (const w of state.villagers) {
    ...
    if (killer && killer.alive) {
      ...
      const heardIt =
        (peekMind(w.id)?.episodic.some((e) => e.kind === 'sawKill' && e.subjectId === killer.id) ?? false) ||
        w.memories.some((m) => m.kind === 'sawKill' && m.subjectId === killer.id)
  ...
  if (victim.homeOwnerId === victim.id) {
    ...
    if (victim.spouseId !== null) {
      const spouse = state.villagers.find((s) => s.alive && s.id === victim.spouseId)
      if (spouse) heir = spouse
    }
    if (!heir) {
    ...
    // Genetic / adoptive children even if never seated as homeOwnerId residents
    // (birth used to orphan kids when beds were full — lineage must still inherit).
    if (!heir) {
      for (const r of state.villagers) {
        if (!r.alive || r.id === victim.id) continue
        if (r.parentIds.includes(victim.id) || r.adoptiveParentIds.includes(victim.id)) {
          heir = r
          break
        }
      }
    ...
    if (heir) {
      transferHomeState(victim, heir)
      heir.hasCart = heir.hasCart || victim.hasCart
      for (const r of state.villagers) {
        if (r.alive && r.id !== heir.id && r.homeOwnerId === victim.id) r.homeOwnerId = heir.id
      }
      ...
      if (victim.hasField && victim.fieldX >= 0) claimArea(state.grid, victim.fieldX, victim.fieldY, FIELD_RADIUS, CLAIM_NONE)
      ...
      for (const r of state.villagers) {
        if (r.alive && r.homeOwnerId === victim.id) {
          r.hasHome = false
          r.homeOwnerId = null
          r.homeX = -1
          r.homeY = -1
...
export function creditRescue(state: SimState, saved: Villager, rescuer: Villager) {
  ...
  recordHelp(state, 'defend', rescuer.id)
  noteHelpOutcome(state, 'defend', rescuer.id)
