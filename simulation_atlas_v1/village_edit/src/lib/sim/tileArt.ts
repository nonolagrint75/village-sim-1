...
import { WHEAT_RIPE } from '@/lib/sim/behaviors'
import { natureHandlesTerrain, naturePixel32, type SeasonName } from '@/lib/render/nature'
...
export function tilePixel32(
  ...
  const naturePacked = naturePixel32(terrain, amount, x, y, biomeId, season, terrainGrid, worldW)
  if (naturePacked != null) return naturePacked
  ...
  if (vary) {
    ...
    } else if (kind === TRAIL || kind === PATH || kind === ROAD) {
      const j = ((n - 0.5) * 28) | 0
      const wear = kind === ROAD ? 16 : kind === PATH ? 10 : 4
      r = clampByte(r + j + wear - 4)
      g = clampByte(g + j + ((n3 * 10) | 0) - (wear >> 1))
...
export function drawCloseupTerrain(
  ...
  const detail = tileS >= 8
  const groundDetail = tileS >= 7 && tiles <= 11000
  const elevShade = tileS >= 7.5 && tiles <= 11000
  const shimmer = tileS >= 10 && tiles <= 9000
  const foam = tileS >= 11 && tiles <= 7000
  ...
  for (let gy = y0; gy < y1; gy++) {
    ...
    for (let gx = x0; gx < x1; gx++) {
      ...
      if (natureHandlesTerrain(t, amount[row + gx])) continue
