...
export function emotionTaskBias(e: EmotionState, kind: string): number {
  ...
  if (kind === 'gatherFood' || kind === 'harvestWheat' || kind === 'fish' || kind === 'takeFromChest') {
    m *= 1 + e.stress * 0.18 + Math.max(0, -e.valence) * 0.12
  }
  if (kind === 'entertain') m *= 1 + e.affection * 0.25 + e.pride * 0.15 - e.stress * 0.2
  if (kind === 'teachCraft') m *= 1 + e.pride * 0.35 + e.affection * 0.2 - e.stress * 0.18
  if (kind === 'buildHouse' || kind === 'tradeRun' || kind === 'craftIronTool') m *= 1 + e.pride * 0.25
  // Curiosity / approach → live TaskKinds (no TaskKind `explore`; wander = idle, invent = experiment).
  if (kind === 'idle') m *= 1 + Math.max(0, e.approachAvoid) * 0.35 + Math.max(0, e.valence) * 0.12
  if (kind === 'experiment') {
    m *= 1 + Math.max(0, e.approachAvoid) * 0.45 + Math.max(0, e.valence) * 0.2 + e.pride * 0.1
  }
  if (e.stress > 0.65 && (kind === 'idle' || kind === 'experiment' || kind === 'tameHorse')) m *= 0.55
  // Valence: low mood dampens exploratory / social novelty
  if (e.valence < -0.35 && (kind === 'idle' || kind === 'experiment' || kind === 'tameHorse')) m *= 0.75
  if (e.approachAvoid < -0.4 && (kind === 'socialise' || kind === 'tradeRun' || kind === 'teachCraft')) m *= 0.7
  return m
}
