...
export function senseResource(
  ...
  opts: {
    ...
    shortR?: number
    allowBlind?: boolean
    /** Legacy v.memories — fallback only; mind semantic/episodic preferred via knownSpots. */
    legacy?: Memory[] | null
    /** When set, reinforce episodic spots on memory recall (decision rehearsal). */
    tick?: number
  } = {},
  ...
  // Mind semantic/episodic first; legacy memories only as fallback inside knownSpots.
  ...
  if (bestMem) {
    if (opts.tick != null) reinforceRecall(mind.episodic, 'goodSpot', null, opts.tick, { x: bestMem.x, y: bestMem.y, r: 18 })
...
export function spotMemoryBias(
  ...
  let bias = 0
  let recalledGood = false
  let recalledDanger = false
  ...
  if (tick != null) {
    if (recalledGood) reinforceRecall(mind.episodic, 'goodSpot', null, tick, { x, y, r: radius })
    if (recalledDanger) {
      reinforceRecall(mind.episodic, 'dangerSpot', null, tick, { x, y, r: radius })
      reinforceRecall(mind.episodic, 'sawKill', null, tick, { x, y, r: radius })
    }
  }
  return bias
}
