...
export function updateNeeds(state: SimState, v: Villager, needs: NeedPressures): void {
  ...
  needs.hunger = clamp01(
    1 - v.hunger / HUNGER_MAX + (food < 1 ? 0.25 : 0) + (villagerFeelsFamine(state, v) ? 0.15 : 0),
  )
  ...
  // Phase A: household pantry / homeless kin → hunger + shelter + belonging (LOD every ~10 ticks).
  if ((state.tick + v.id) % 10 === 0) {
    const hh = getHouseholdNeeds(state, v)
    needs.hunger = clamp01(needs.hunger + hh.food * 0.22)
    needs.shelter = clamp01(needs.shelter + hh.shelter * 0.28)
    needs.belonging = clamp01(needs.belonging + (hh.homelessKin > 0 ? 0.12 : 0) + hh.food * 0.08)
    if (hh.food > 0.45 && valuesFromPersonality(v).family > 0.4) {
      needs.social = clamp01(needs.social + 0.1)
    }
  }
