...
export type PredictionErrorKind =
  ...
  | 'darkness'
  | 'cold_comfort'
  /** Wave B: |model expected − observed| after task outcome. */
  | 'task_model'
...
/**
 * Wave B lite 1-step world-model hint — not P(S'|S,A) full, not MCTS.
 * Stored lightly when an act is chosen; compared to outcome for model PE.
 */
export interface PredictedOutcome {
  kind: TaskKind | null
  /** Heuristic P(success) 0–1. */
  success: number
  /** Local danger / badness 0–1. */
  danger: number
  /** Expected scalar reward ≈ success × (1 − k·danger). */
  expected: number
}
...
export interface LivelihoodProfile {
  ...
  /** Last soft profession change (career switch cooldown). */
  lastCareerChangeTick: number
...
export interface CognitiveState {
  ...
  /**
   * Wave B: last chosen-act forecast from lite world model.
   * Used for imagination tweak + model PE (expected − observed).
   */
  predictedOutcome: PredictedOutcome
  /**
   * Wave B: signed model PE (expected − observed), decays each tick.
   * Negative = worse than forecast; positive = better.
   */
  modelPredictionError: number
