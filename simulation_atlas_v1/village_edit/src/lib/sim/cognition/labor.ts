...
export function preferenceForTask(kind: string): LaborPref | null {
  ...
  if (kind === 'gatherFuel' || kind === 'tendHearth' || kind === 'lightTorch' || kind === 'placeCandle') {
    return kind === 'gatherFuel' ? 'woodwork' : 'crafting'
  }
...
export function preferenceTaskBias(prefs: LaborPreferences, kind: string): number {
  ...
  // Widen vs catalogue baseScore dominance (was 0.72 + like*0.83 → 0.72…1.55).
  // 0 → 0.62, ~0.35 → 1.0, 1 → 1.72 — still soft, not a hard gate.
  return 0.62 + like * 1.1
}
...
/** Primary procedural skills that define a métier (guild apprenticeship drip). */
export function primarySkillsForProfession(job: Profession): ProceduralSkill[] {
  switch (job) {
    case 'lumberjack':
      return ['chop']
    case 'fisher':
