...
export function nearestTacticalThreat(state: SimState, v: Villager, radius: number): TacticalThreat | null {
  ...
  // Ally combat contagion — spatial hash (was O(N²) full villager scan @500).
  // Under severe Max load, skip contagion; wolves/bandits above still fire.
  const perf = getSimPerfBudget()
  if (perf.lastTps > 0 && perf.lastTps < 40 && perf.headroom < 0.2) return null

  const senseR = radius * 1.1
  const near = nearbyVillagers(state, v.x, v.y, senseR, v.id, 24)
