 ...
 * Lot 1 port from manual UI — adapted to canon SimStats (optional extras via duck-typing).
...
function optNum(stats: SimStats, key: string): number {
...
export function samplePoint(stats: SimStats): TelemetryPoint {
  return {
    ...
    fields: stats.fields,
    mills: stats.mills,
    // Canon SimStats may omit wells / plaza / survival avgs — sample 0 until Lot 2 wires producers.
    wells: optNum(stats, 'wells'),
    plazaFires: optNum(stats, 'plazaFires'),
    ...
    avgHunger: optNum(stats, 'avgHunger'),
    avgThirst: optNum(stats, 'avgThirst'),
    avgEdible: optNum(stats, 'avgEdible'),
    homeless: optNum(stats, 'homeless'),
...
export function pushSample(rec: LiveRunRecorder, stats: SimStats, chronicle: string[]): void {
  ingestChronicle(rec, chronicle)
  const day = dayIndexOf(stats)
  if (day === rec.lastDay && rec.series.length > 0) return
  rec.lastDay = day
...
export function ingestChronicle(rec: LiveRunRecorder, lines: string[]): void {
  if (!lines.length) return
  let start = 0
  if (rec.lastLogLine) {
    const idx = lines.lastIndexOf(rec.lastLogLine)
    start = idx >= 0 ? idx + 1 : 0
  }
  for (let i = start; i < lines.length; i++) {
    const line = lines[i]
    if (!line) continue
    if (rec.chronicleTail[rec.chronicleTail.length - 1] === line) continue
    rec.chronicleTail.push(line)
  }
  if (rec.chronicleTail.length > MAX_CHRONICLE_TAIL) {
    rec.chronicleTail.splice(0, rec.chronicleTail.length - MAX_CHRONICLE_TAIL)
  }
  rec.lastLogLine = rec.chronicleTail[rec.chronicleTail.length - 1] ?? rec.lastLogLine
}
...
export function inferDeathCauses(lines: string[]): RunCauseBucket {
  ...
  for (const line of lines) {
    ...
    else if (/loup|devor|dévor/.test(t)) bump('loup')
    else if (/vieillesse/.test(t)) bump('vieillesse')
    else if (/chaleur|succombe/.test(t) && /chaleur/.test(t)) bump('chaleur')
    else if (/maladie/.test(t)) bump('maladie')
    else if (/bandit|brigand/.test(t)) bump('bandit')
...
export function archivesSummaryText(list: SimRunArchive[]): string {
  if (!list.length) return 'Aucune simulation archivee.'
  return list
    .map((a, i) => {
      const s = a.summary
      const causes = Object.entries(s.causes)
        ...
        .map(([k, n]) => `${k}:${n}`)
        .join(' ')
      return `#${i + 1} seed=${a.seed} j${s.days} pop ${s.finalPop}/${s.maxPop} +${s.deaths} nes${s.births} maisons${s.houses} [${causes || '-'}]`
    })
    .join('\n')
}