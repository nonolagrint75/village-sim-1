...
import type { BuildBlock, BuildBlockKind, BuildMaterial } from './homeContracts'
...
export interface GridChunkCoord {
  chunkX: number
  chunkY: number
  localX: number
  localY: number
...
export function cellToChunk(x: number, y: number, chunkSize = CHUNK_SIZE): GridChunkCoord {
...
export function chunkIdOf(coord: GridChunkCoord): ChunkId {
