...
export function planHomeSpatial(brief: HomePlannerBrief, rng: () => number): SpatialHomePlan {
  ...
  // Pioneer lean: clamp round/ell/courtyard only while still wood-poor AND
  // small household AND low wealth — keeps 0-wood camps on simple shells.
  // Escape (any): woodOnHand >= 12 OR household >= 3 OR wealth >= 8.
  const pioneerShapeLean =
    brief.mode === 'new' &&
    brief.woodOnHand < 12 &&
    brief.household < 3 &&
    brief.wealth < 8
