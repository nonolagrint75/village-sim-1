...
import type { BuildBlock, DoorSide, SpatialHomePlan } from './homeContracts'
...
export function nextBuildBlock(queue: BuildBlock[], grid?: WorldGrid): BuildBlock | null {
  for (const b of queue) {
    if (b.done) continue
    // Out-of-bounds stays unfinished so callers can reject (do not silent-complete).
    if (grid && inBounds(grid, b.x, b.y) && !blockNeedsWork(grid, b)) {
      b.done = true
      continue
    }
    return b
