/**
 * Lot 3B — chantier coherence cleanup (adapted from deferred).
 * Not wired into engine tick by default — call sites opt in.
 * Does not scrap legitimate finished homes.
 */
...
import type { SpatialHomePlan } from './homeContracts'
...
/** Drop finished blocks from queue (keeps unfinished order). */
export function pruneCompletedBuildQueue(queue: import('./homeContracts').BuildBlock[]): import('./homeContracts').BuildBlock[] {
  return queue.filter((b) => !b.done)
}

/** Clear queue + plan without wiping finished homes. */
export function clearAbandonedBuildState(v: import('../types').Villager): void {
  if (v.hasHome) return
  v.homePlan = null
  v.buildQueue = []
}