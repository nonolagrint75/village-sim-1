...
export function pickGearCraftTarget(
  ...
  opts: {
    ...
    cold01: number
    canCraft: CraftGate
    /** Optional household stock (bag+chest); defaults to bag only. */
    stockOf?: (res: ResourceType) => number
  },
): GearId | null {
  const eq = ensureEquipment(v)
  const coins = countOf(v.inventory, 'coin')
  ...
  const stock = (res: ResourceType) => (opts.stockOf ? opts.stockOf(res) : countOf(v.inventory, res))
