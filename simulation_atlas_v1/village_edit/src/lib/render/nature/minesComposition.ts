 ...
 * Live mine composition (from staging mines_ore/art).
 *
...
export function expandMineSite(
  ...
  // Eye-QA: ore identity near mines must read at a glance (Factorio mounds beside mouth).
  const piles = Math.max(site.ore && site.ore !== "stone" ? 2 : 1, Math.min(3, site.pileCount ?? (site.ore ? 2 : 1)))
