...
import { distance, findMillSite, findNearbyShore, getTerrain, inBounds, resourceDensity } from './world'
...
/** Soft workshop surplus from specialty (wood / grain / fish / ore / craft). */
function applyRegionalProduction(state: SimState, village: Village) {
  ...
  if (spec === 'forest') {
    ...
    boost('wheat', 0.5)
    boost('flour', village.hasMill ? 0.35 : 0.08)
    boost('bread', village.hasMill ? 0.25 : 0.05)
  } else if (spec === 'shore') {
    boost('food', 0.55)
  } else if (spec === 'mine') {
    boost('iron', 0.4)
...
export function villageBirthBias(village: Village | undefined | null): number {
  if (!village) return 1
  return clampNum(0.72 + (village.prosperity ?? 35) / 180 + (village.loyalty ?? 0.5) * 0.2, 0.55, 1.35)
}
...
export function tickMarketPrices(state: SimState) {
  ...
  const famineFood = state.famine ? 1.35 : 1
  ...
  for (const res of TRACKED_RESOURCES) {
    ...
    state.prices[res] = Math.round((prev * (1 - smooth) + cleared * smooth) * 10) / 10
  }
}
...
export function findTradeOpportunity(
  ...
  for (const other of state.villages) {
    ...
    for (const resource of TRACKED_RESOURCES) {
      ...
      const score = (gain / targetOf(resource)) / (1 + effD * 0.0035)
      if (score > bestScore) {
        bestScore = score
