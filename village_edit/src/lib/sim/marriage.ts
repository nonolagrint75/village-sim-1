...
/** Âge min (ticks) pour se lier / se reproduire socialement. */
export const MARRY_MIN_AGE = 220
/** Enfant / adolescent encore adoptable. */
export const ADOPT_MAX_AGE = 200
...
export function formBond(
  ...
  if (rng() < 0.55 || !state.milestones.firstMarriage) {
...
export function tryRomanceBond(state: SimState, a: Villager, b: Villager, rng: () => number): boolean {
  ...
  if (rel.affinity < ROMANCE_AFFINITY || rel.trust < ROMANCE_TRUST) return false
  if (scored.love < 0.45) return false
  // Soft chance — not automatic on every chat
  if (rng() > 0.12 + scored.love * 0.18 + mindOf(a).values.family * 0.1) return false
...
function pickBestCandidate(
  ...
  for (const b of state.villagers) {
    ...
    const scored = scoreMate(state, a, b, lookup)
    if (scored.blocked || scored.score < 0.85) continue
    if (prefer && scored.pathway !== prefer && scored.score < 1.2) continue
...
export function tickMarriage(state: SimState, rng: () => number) {
  ...
  for (const v of state.villagers) {
    ...
    maybeRefuseMarriage(state, v, rng)
    if (v.refusesMarriage || !isMarriageAge(v)) continue

    const pol = politicsOf(v)
    const mind = mindOf(v)
    const elderPush = circlesOf(state, v).some((c) => c.kind === 'elder') && pol.beliefs.tradition > 0.48
    const familyPush = v.ambition === 'family' || mind.values.family > 0.65
    const wealthPush =
      (v.ambition === 'wealth' || mind.values.wealth > 0.62 || pol.beliefs.greed > 0.55) &&
      mind.values.family < 0.7

    // Arranged-ish
    if ((elderPush || (familyPush && pol.beliefs.tradition > 0.4)) && rng() < 0.08 + pol.beliefs.tradition * 0.1) {
      const pick = pickBestCandidate(state, v, lookup, 'arranged')
      if (pick && pick.scored.pressure > 0.35) {
        if (formBond(state, v, pick.other, 'arranged', rng)) {
          formed++
          if (formed >= 2) return
          continue
        }
    ...
    // Wealth marriage
    if (wealthPush && rng() < 0.06 + mind.values.wealth * 0.08) {
      const pick = pickBestCandidate(state, v, lookup, 'wealth')
      if (pick && pick.scored.wealthPull > 0.4) {
...
export function tickAdoption(state: SimState, rng: () => number) {
  ...
  if (state.famine && rng() > 0.35) return // rarer in famine, but orphans more likely
