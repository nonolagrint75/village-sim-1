...
| Global Workspace (Baars/Dehaene) | `workspace.ts` + `consciousness.ts` | **OUI** |
| Dual-process S1/S2 | `executive.ts` `processMode` | **OUI** |
| Prédiction / interoception | `predictive.ts` | **OUI** |
| RPE / apprent. habitudes | `recordTaskOutcome` + mindPool | **OUI** |
| Mémoires épi / sém / procéd. | `memory.ts` | **OUI** |
| Sélection d’action (softmax) | `decide.ts` → `chooseTask` | **OUI** |
| Theory of Mind | `tom.ts` | **OUI** (1er ordre) |
...
```mermaid
flowchart TD
  World[Monde perçu] --> Needs[Besoins / PE]
