...
export function resetPathBudget(aliveAgents = 40) {
  ...
  // ~9 searches at 40 agents → ~16 at 400 (sublinear), then TPS scale.
  searchesLeft = Math.max(4, Math.min(16, Math.round((SEARCHES_PER_TICK_BASE + ((a / 50) | 0)) * scale)))
  // Cap total node expansions so one tick cannot freeze the worker.
  nodesLeft = Math.max(8000, Math.min(28000, Math.round((NODES_PER_TICK_BASE + ((a * 18) | 0)) * scale)))
}
...
export function navigate(
  ...
  if (needPath) {
    ...
    const shortOpen = dist <= 3 && !profile.amphibious && kingPathClear(grid, actor.x, actor.y, tx, ty, profile)
    if (shortOpen) {
      cache.path = null
      cache.pathI = 0
      cache.pathTx = tx
